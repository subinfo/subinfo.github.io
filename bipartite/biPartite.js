...
if (id=="dim_dim" || id =="dim_reto") {
	...
	colors=colors2;
		...
		mainbar.append("rect").attr("class","mainrect")
			.attr("id",function(d,i){ return data.keys[p][i];})
		...
		mainbar.append("text").attr("class","barlabel")	
			...
			.attr("fill",function(d,i){if(/^[0-9].[0-9]/.test(data.keys[p][i])){return "blue"};});
	...
	bP.selectSegment = function(id, data, m, s){
		...
		data.forEach(function(k){
			...
			newdata.data[n] = k.data.data[n].map( function(d){ return d;});
			
			newdata.data[1-n] = k.data.data[1-n]
