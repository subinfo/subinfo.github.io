!function(){
	...
	var colors3 =["#E8BA20","#E8BA20", "#D47F23","#D47F23", "#AE007E", "#673C8B","#673C8B", "#009BDB", "#7C7B7E", "#0095B0"];
	...
	var colors2=["#E8BA20", "#D47F23", "#AE007E", "#673C8B", "#009BDB", "#7C7B7E", "#0095B0"];
	//var colors2 =["#e5233d","#dda73a","#4ca146","#c7212f","#ef402d","#27bfe6","#fbc412","#a31c44","#f26a2e","#de1768","#f89d2a","#bf8d2c","#407f46","#1f97d4","#59ba47","#136a9f","#14496b"];
...
//}
		...
		mainbar.append("rect").attr("class","mainrect")
			.attr("id",function(d,i){ if(/^[0-9]/.test(data.keys[p][i])){return "ods"+data.keys[p][i][0]}else{return data.keys[p][i]};})
		...
		d3.select("#"+id).select(".part"+p).select(".subbars")
			...
			.style("fill",function(d,i){console.log(data.keys[0][i]); if(id=="dim_dim2"){return colors[data.keys[0][1][0]-1]}else{return colors[d.key1]};});
	}
...
if (id=="dim_ods") {
	...
	colors=colors2;
		...
		d3.select("#"+id).select(".edges").selectAll(".edge")
			.data(data.edges).enter().append("polygon").attr("class","edge")
			.attr("points", edgePolygon).style("fill",function(d){return colors[data.keys[0][1][0]-1];})
			.style("opacity",0.5).each(function(d) { this._current = d; });	
	}	
}
		...
		data.forEach(function(biP,s){
			...
			[0,1].forEach(function(p){		
				...
				d3.select("#"+biP.id)
					...
					.select(".mainbars")
					.selectAll(".mainbar")
					.on("mouseover",function(d, i){return bP.selectSegment(biP.id, data, p, i); })
					.on("mouseout",function(d, i){ return bP.deSelectSegment(data, p, i); });	
			});
		});	
	}	
	
	bP.selectSegment = function(id, data, m, s){
		var n=0;
		...
		data.forEach(function(k){
			...
			if ((((id=="dim_dim") || (id=="dim_reto" && m==0) || (id=="reto_dim" && m==1)) && k.id=="reto_reto") || (((id=="reto_reto") || (id=="dim_reto" && m==1) || (id=="reto_dim" && m==0)) && k.id=="dim_dim")) {
				return;
			}

			if ((id=="dim_dim" && ((m==0 && k.id=="reto_dim") || (m==1 && k.id=="dim_reto"))) || (id=="reto_reto" && ((m==1 && k.id=="reto_dim") || (m==0 && k.id=="dim_reto"))) || (id=="dim_reto" && ((m==0 && k.id=="reto_dim") || (m==1 && (k.id=="reto_reto" || k.id=="reto_dim" )))) || (id=="reto_dim" && ((m==0 && k.id=="dim_reto") || (m==1 && (k.id=="dim_dim" || k.id=="dim_reto" ))))) {
				n=1-m;
			}else{
				n=m;
			}
