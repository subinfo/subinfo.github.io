...
//}
		...
		d3.select("#"+id).select(".part"+p).select(".subbars")
			...
			.style("fill",function(d,i){console.log(data.keys[0][i]); if(id=="dim_dim2"){return colors[data.keys[0][0][0]-1]}else{return colors[d.key1]};});
	}
...
if (id=="dim_ods") {
	...
	colors=colors2;
		...
		d3.select("#"+id).select(".edges").selectAll(".edge")
			.data(data.edges).enter().append("polygon").attr("class","edge")
			.attr("points", edgePolygon).style("fill",function(d){return colors[data.keys[0][0][0]-1];})
			.style("opacity",0.5).each(function(d) { this._current = d; });	
	}	
}
