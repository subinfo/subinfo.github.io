...
if (id=="dim_ods") {
	...
	colors=colors2;
		...
		mainbar.append("rect").attr("class","mainrect")
			.attr("id",function(d,i){ if(/^[0-9]/.test(data.keys[p][i])){return "ods"+data.keys[p][i][0]}else{return "ods1"+data.keys[p][i][1]};})
		...
		d3.select("#"+id).select(".part"+p).select(".subbars")
			...
			.style("fill",function(d,i){console.log(data.keys[0][i]); if(id=="dim_ods"){return colors1[data.keys[0][i][0]-1]}else{return colors3[d.key1]};});
	}
...
if (id=="dim_ods") {
	...
	colors=colors2;
		...
		d3.select("#"+id).select(".edges").selectAll(".edge")
			...
			.attr("points", edgePolygon).style("fill",function(d){return colors3[d.key1];})
